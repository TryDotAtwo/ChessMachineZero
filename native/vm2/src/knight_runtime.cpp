...
torch::Tensor select_knight_legality_tokens(const KnightCircuit& c,const torch::Tensor& state){return torch::matmul(torch::matmul(c.legal_row,state),c.legality_features);}
