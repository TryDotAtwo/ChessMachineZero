...
KnightCircuit compile_knight_circuit(){
 auto x=torch::zeros({N,D},torch::kFloat64);x.index_put_({Q,ONE},1.);
 ...
 auto lr=torch::zeros({1,N},torch::kFloat64);lr.index_put_({0,Q},1.);auto lf=torch::zeros({D,1},torch::kFloat64);lf.index_put_({LEGAL+1,0},1.);auto lfs=torch::zeros({D,2},torch::kFloat64);lfs.index_put_({LEGAL,0},1.);lfs.index_put_({LEGAL+1,1},1.);auto br=torch::zeros({64,N},torch::kFloat64);for(std::int64_t i=0;i<64;++i)br.index_put_({i,OUT+i},1.);auto bf=torch::zeros({D,5},torch::kFloat64);for(int i=0;i<5;++i)bf.index_put_({OUT_PIECE+i,i},1.);auto sr=torch::zeros({1,N},torch::kFloat64);sr.index_put_({0,Q},1.);auto sf=torch::zeros({D,2},torch::kFloat64);sf.index_put_({OUT_SIDE_F,0},1.);sf.index_put_({OUT_SIDE_F+1,1},1.);
 return KnightCircuit{freeze(x),wq,wk,wv,masks,rows,features,freeze(lr),freeze(lf),freeze(lfs),freeze(br),freeze(bf),freeze(sr),freeze(sf)};
}

torch::Tensor bind_knight_input(const KnightCircuit& c,const KnightBoard& b,std::int64_t source,std::int64_t target){if(source<0||source>=64||target<0||target>=64)throw std::out_of_range("knight square outside board");auto x=c.tokens.clone();x.index_put_({Q,SF+source%8},1.);x.index_put_({Q,TF+target%8},1.);x.index_put_({Q,SR+source/8},1.);x.index_put_({Q,TR+target/8},1.);x.index_put_({Q,ADDR+source},1.);x.index_put_({Q,TADDR+target},1.);x.index_put_({Q,SIDE+static_cast<std::int64_t>(b.side)},1.);for(std::int64_t sq=0;sq<64;++sq)x.index_put_({SQUARE+sq,PIECE+static_cast<std::int64_t>(b.squares[sq])},1.);return freeze(x);}
} // namespace cmz::vm2
