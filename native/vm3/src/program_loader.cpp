...
FrozenChessProgram load_frozen_program(const std::filesystem::path& path, const torch::Device& device) {
  ...
  auto read_block=[&](){Attention2dBlock b;b.query_router=tensor_ref();b.key_router=tensor_ref();b.output_router=tensor_ref();b.fixed_mask=tensor_ref();b.fixed_eligibility=tensor_ref();return b;};
  auto read_stage=[&](){FrozenStage s;s.wq=tensor_ref();s.wk=tensor_ref();s.wv=tensor_ref();s.fixed_mask=tensor_ref();s.row_router=tensor_ref();s.feature_router=tensor_ref();auto count=r.value<std::uint32_t>();if(count>100000)Reader::fail("attention block count");for(std::uint32_t i=0;i<count;++i)s.attention_plan.blocks.push_back(read_block());return s;};
