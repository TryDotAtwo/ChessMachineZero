...
torch::Tensor compile_pawn_geometry(const CandidateBank& bank) {
  ...
  for (const auto& move : bank.moves) {
    ...
    if (target_rank != 7 && file_delta == 0 && rank_delta == 1)
      result.index_put_({id, 0}, 1.0);
    if (source_rank == 1 && file_delta == 0 && rank_delta == 2)
      result.index_put_({id, 1}, 1.0);
    if (target_rank != 7 && std::abs(file_delta) == 1 && rank_delta == 1)
      result.index_put_({id, 2}, 1.0);
    if (target_rank != 0 && file_delta == 0 && rank_delta == -1)
      result.index_put_({id, 3}, 1.0);
    if (source_rank == 6 && file_delta == 0 && rank_delta == -2)
      result.index_put_({id, 4}, 1.0);
    if (target_rank != 0 && std::abs(file_delta) == 1 && rank_delta == -1)
