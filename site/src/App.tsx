...
function KnightLab(){ const [source,setSource]=useState(1),[target,setTarget]=useState<number|null>(18); const t=target===null?null:knightTrace(source,target),targets=knightTargets(source); const click=(sq:number)=>{if(target!==null){setSource(sq);setTarget(null)}else setTarget(sq)}; return <section className="knight-lab"><div className="lab-copy"><small>СДЕЛАНО ПО ИДЕЕ PERCEPTA</small><h2>{target===null?`Источник ${squareName(source)} выбран — кликните цель`:"Конь: кликните источник, затем цель"}</h2><p>Каждая выбранная пара немедленно пересобирает вычисляемые токены и матрицы справа. Голубые точки — цели, которые классифицирует этот же контур.</p><div className="knight-board">{boardSquares.map(sq=><button key={sq} onClick={()=>click(sq)} className={`${(Math.floor(sq/8)+sq%8)%2?"dark":"light"} ${sq===source?"source":""} ${sq===target?"target":""} ${targets.includes(sq)?"reachable":""}`} aria-label={squareName(sq)}><span>{sq===source?"♞":""}</span><small>{squareName(sq)}</small></button>)}</div>{t?<div className={`verdict ${t.winner===1?"legal":"illegal"}`}><span>{squareName(source)} → {squareName(target!)} · Δfile={t.delta[0]}, Δrank={t.delta[1]}</span><strong>{t.winner===1?"ход коня":"не ход коня"}</strong></div>:<div className="verdict waiting"><span>Источник: {squareName(source)}</span><strong>ожидаю цель…</strong></div>}<p className="boundary">Пока доказана геометрия. Цвет, занятость и применение хода ещё не подключены.</p></div><div className="explain-flow"><h3>Трасса выбранной пары</h3>{t?<ol key={`${source}-${target}`}><li><b>1. Координаты → токен X</b><span>{squareName(source)} → {squareName(target!)}: Δfile={t.delta[0]}, Δrank={t.delta[1]}; X = [{t.token.join(", ")}].</span><TinyMatrix label="X (меняется)" rows={[t.token]}/></li><li><b>2. X·Wq → запрос Q</b><span>Wq — фиксированные веса правила; Q пересчитан для этой пары: [{t.q.join(", ")}].</span><div className="step-matrices"><TinyMatrix label="Wq (фикс.)" rows={WQ}/><TinyMatrix label="Q (меняется)" rows={[t.q]}/></div></li><li><b>3. Q·Kᵀ → два score</b><span>Kᵀ — фиксированные ключи классов. Scores для текущих клеток: [{t.scores.join(", ")}].</span><TinyMatrix label="Kᵀ (фикс.)" rows={CLASS_KEYS}/></li><li className="winner-step"><b>4. Hardmax → класс геометрии</b><span>{t.scores[0]} против {t.scores[1]} → {t.winner?"KNIGHT_GEOMETRY":"NOT_KNIGHT"}.</span><div className="hardmax-pulse">hardmax → {t.winner}</div></li></ol>:<div className="trace-wait"><strong>Выберите цель на доске</strong><span>После клика здесь появятся X, Q, scores и hardmax именно для пары {squareName(source)} → цель.</span></div>}<details><summary>Показать фиксированную Wk</summary><TinyMatrix label="Wk (фикс.)" rows={WK}/></details></div></section>; }
...
export default function App(){
 ...
 <section className="provenance"><span>schema {trace.schema}</span><span>commit {trace.provenance.commit.slice(0,12)}</span><span>float64</span><span>production: frozen block-sparse attention</span></section>
 <section className="optimization"><div><small>ЛОГИЧЕСКИЕ SCORE-ПАРЫ</small><strong>15 120 240</strong></div><div><small>КОНЕЧНЫЕ ПАРЫ МАСОК</small><strong>203 861</strong></div><div><small>ФИЗИЧЕСКИЕ БЛОКИ</small><strong>≤ 16 / stage</strong></div><div><small>CPU TRANSITION MEDIAN</small><strong>1346.7 → 1007.3 ms</strong></div><p>Логическая матрица сохраняется для проверки. Production inference исполняет offline-собранные прямоугольные блоки; запрещённые пары остаются −∞. GPU speedup ещё не измерен.</p></section>
