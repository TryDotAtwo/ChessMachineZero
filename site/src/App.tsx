...
import { knightTargets, knightTrace, squareName, WQ, WK, CLASS_KEYS } from "./knight";
...
function KnightLab(){ const [source,setSource]=useState(1),[target,setTarget]=useState(18),[pickSource,setPickSource]=useState(true); const t=knightTrace(source,target),targets=knightTargets(source); const click=(sq:number)=>{if(pickSource){setSource(sq);setTarget(sq);setPickSource(false)}else{setTarget(sq);setPickSource(true)}}; return <section className="knight-lab"><div className="lab-copy"><small>СДЕЛАНО ПО ИДЕЕ PERCEPTA</small><h2>Конь: кликните источник, затем цель</h2><p>Доска выбирает два токена. Подсветка показывает клетки, которые матричный контур классифицирует как геометрию коня.</p><div className="knight-board">{Array.from({length:64},(_,i)=>63-i).map(sq=><button key={sq} onClick={()=>click(sq)} className={`${(Math.floor(sq/8)+sq%8)%2?"dark":"light"} ${sq===source?"source":""} ${sq===target?"target":""} ${targets.includes(sq)?"reachable":""}`} aria-label={squareName(sq)}><span>{sq===source?"♞":""}</span><small>{squareName(sq)}</small></button>)}</div><div className={`verdict ${t.winner===1?"legal":"illegal"}`}><span>{squareName(source)} → {squareName(target)} · Δfile={t.delta[0]}, Δrank={t.delta[1]}</span><strong>{t.winner===1?"ход коня":"не ход коня"}</strong></div><p className="boundary">Пока доказана геометрия. Цвет, занятость и применение хода ещё не подключены.</p></div><div className="explain-flow"><h3>Что сейчас произошло</h3><ol><li><b>1. Пара клеток стала токеном</b><span>{squareName(source)} → {squareName(target)} кодируется как X = [{t.token.join(", ")}].</span><TinyMatrix label="X" rows={[t.token]}/></li><li><b>2. Токен спроецирован в запрос</b><span>X·Wq даёт Q = [{t.q.join(", ")}].</span><div className="step-matrices"><TinyMatrix label="Wq" rows={WQ}/><TinyMatrix label="Q" rows={[t.q]}/></div></li><li><b>3. Запрос сравнился с двумя классами</b><span>Классы: «не конь» и «конь». Q·Kᵀ даёт scores = [{t.scores.join(", ")}].</span><TinyMatrix label="Kᵀ" rows={CLASS_KEYS}/></li><li className="winner-step"><b>4. Hardmax выбрал больший score</b><span>{t.scores[0]} против {t.scores[1]} → класс {t.winner}: {t.winner?"KNIGHT_GEOMETRY":"NOT_KNIGHT"}.</span><div className="hardmax-pulse">hardmax → {t.winner}</div></li></ol><details><summary>Показать Wk</summary><TinyMatrix label="Wk" rows={WK}/></details></div></section>; }
