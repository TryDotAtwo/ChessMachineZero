...
- graph_executor=FrozenVm requires an artifact and CUDA device, validates SSA/op schemas before execution, materializes immutable FP4 tensors once, and dispatches only generic tensor operators; artifact-declared row routing and GEMM are CUDA/autograd verified, while unbound HullAttention2D is rejected rather than substituted
