...
- [x] Commit scoped changes and publish with an ordinary fast-forward push to the already agreed remote main only after checks and remote-state verification. No force push, dirty root mutation or implicit unrelated merge. Verify Pages deployment status and live content; report failed/unverified deployment honestly if blocked.
