"""Weight-compiled rules plus learned strategy shell."""
...
from chess_machine_zero.model.weight_compiled_rules import (
    WeightCompiledRulesTransformer,
    board_state_from_prompt_trace,
...
class WeightCompiledMachineDecision:
...
@dataclass(frozen=True, slots=True)
class WeightCompiledGameRecord:
    start_fen: str
    final_board: BoardState
    terminal_status: TerminalStatus
    decisions: tuple[WeightCompiledMachineDecision, ...]
    trace: tuple[TracePacket, ...]
...
@dataclass(frozen=True, slots=True)
class CMZWeightCompiledMachine:
    rules: WeightCompiledRulesTransformer
    ...
    def select_move_from_prompt(
        ...
        if not legal_moves:
            raise ValueError("weight-compiled machine requires at least one legal move")
        with torch.no_grad():
        ...
        return WeightCompiledMachineDecision(
            trace=tuple(trace),
    ...
    def play_game_from_fen(
        ...
        decisions: list[WeightCompiledMachineDecision] = []
        ...
        return WeightCompiledGameRecord(
            start_fen=fen,
            final_board=board,
